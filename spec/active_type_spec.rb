...
describe 'ActiveType' do
  before(:all) do
    ActiveRecord::Base.connection.execute <<-SQL
      ...
      CREATE TABLE people (id serial NOT NULL, name varchar, address address,
      CONSTRAINT people_pkey PRIMARY KEY (id ));

      DROP TYPE IF EXISTS many_data_types_type CASCADE;
      DROP TABLE IF EXISTS many_types_models CASCADE;
      CREATE TYPE many_data_types_type AS (binary_type bytea,
        boolean_type boolean, date_type date, datetime_type timestamp,
        decimal_type decimal, float_type float, integer_type integer,
        string_type varchar, text_type text, time_type time,
        timestamp_type timestamp);
      CREATE TABLE many_types_models (id serial NOT NULL, name varchar,
        mdtt many_data_types_type,
          CONSTRAINT manytypesmodel_pkey PRIMARY KEY (id));

      DROP TYPE IF EXISTS project CASCADE;
      DROP TABLE IF EXISTS companies CASCADE;
      CREATE TYPE project AS (name varchar, started timestamp,
        new_project boolean, employees_number integer, some_time time,
        something bytea, project_type varchar);
      CREATE TABLE companies (id serial NOT NULL, name varchar,
        project project, CONSTRAINT company_pkey PRIMARY KEY (id ));

      DROP TYPE IF EXISTS type_with_array CASCADE;
      DROP TABLE IF EXISTS model_with_arrays CASCADE;
      CREATE TYPE type_with_array AS (str varchar, binary_array bytea[],
        boolean_array boolean[], date_array date[], datetime_array timestamp[],
        decimal_array decimal[], float_array float[], integer_array integer[],
        string_array varchar[], text_array text[], time_array time[],
        timestamp_array timestamp[]);
      CREATE TABLE model_with_arrays (id serial NOT NULL, name varchar,
      twa type_with_array, CONSTRAINT modelwitharray_pkey PRIMARY KEY (id ));
      ...
      CREATE TYPE parent_type AS (name varchar, some_datetime timestamp,
        child1 child_type1, child2 child_type2);
      CREATE TABLE some_models (id serial NOT NULL, name varchar,
        parent_type parent_type, CONSTRAINT somemodel_pkey PRIMARY KEY (id));
    SQL
  end
  ...
  describe 'with db' do
    ...
    it 'should work :)' do
      person = Person.create!(name: 'Ivan Groznij',
                              address: Address.new(city: 'Moscow'))
      person.reload
  ...
  describe 'serialization' do
    ...
    it 'serialize a type' do
      ...
      type.pr2 = 'pr2_text'
      type.pr3 = 'pr3_text'
      SomeType.dump(type).should == '("pr1_text","pr2_text","pr3_text")'
    end

    it 'deserialize a type' do
      type = SomeAnotherType.load('(moskow,mohovaya,28)')
      type.one.should == 'moskow'
      type.two.should == 'mohovaya'
      type.three.should == '28'
    end

    it 'raise error when properties not equal on deserialization' do
      expect { SomeThirdType.load('(moskow,mohovaya,28,sometext)') }.
  ...
  describe 'with types' do
    ...
    it 'should work' do
      ...
      mdtt = ManyDataTypesType.new(binary_type: binary_var,
                                   boolean_type: boolean_var,
                                   date_type: date_var,
                                   datetime_type: datetime_var,
                                   decimal_type: decimal_var,
                                   float_type: float_var,
                                   integer_type: integer_var,
                                   string_type: string_var,
                                   text_type: text_var,
                                   time_type: time_var,
                                   timestamp_type: timestamp_var)
      many_model = ManyTypesModel.create!(name: 'some random name', mdtt: mdtt)
      many_model.reload

      #ActiveRecord::Base.connection.unescape_bytea(
      #many_model.mdtt.binary_type.gsub(/\\\\/,'\\')).should
      #== binary_var many_model.mdtt.binary_type.should == binary_var
    ...
    it 'should work even if they are empty' do
      ...
      expect do
        Company.create!(name: 'Cool Company', project:
  ...
  describe 'with arrays' do
    ...
    it 'should work' do
      ...
      date_array = [Date.new(2012, 1, 2), Date.new(2010, 5, 4),
                    Date.new(1995, 9, 1)]
      binary_array = ['one', 'two', 'three']
      datetime_array = [Time.new(2012, 12, 21, 12, 11, 9),
                        Time.new(2013, 9, 11, 15, 11, 9)]
      decimal_array = [BigDecimal.new('0.0001'), BigDecimal.new('0.0002'),
                       BigDecimal.new('0.0005')]
      float_array = [1.54, 5.44, 6.44, 6.99]
      string_array = ['string with comma', 'string comma comma',
                      'string with words']
      text_array = ['text text textext text text', 'text text']
      time_array = [Time.new(2000, 01, 01, 12, 11, 5),
                    Time.new(2000, 01, 01, 12, 11, 5),
                    Time.new(2000, 01, 01, 12, 11, 5)]
      timestamp_array = [Time.new(1999, 4, 14), Time.new(1999, 4, 14),
                         Time.new(1999, 4, 14)]
      twa = TypeWithArray.new(str: str, integer_array: integer_array,
  ...
  describe 'with nested types' do
    ...
    it 'should work' do
      ...
      #child_child = ChildChildType.new(name: child_child_name,
      #some_string: some_string, ccc: ccc)
      child1 = ChildType1.new(name: child1_name, some_date: some_date)
      #, child_child: child_child)
      child2 = ChildType2.new(name: child2_name,
                              some_boolean: some_boolean)
      parent = ParentType.new(name: parent_name,
                              some_datetime: some_datetime,
                              child1: child1, child2: child2)
      model = SomeModel.create!(name: some_model_name,
                                parent_type: parent)
      model.reload
